        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {adjustment?.type_label || (isIncrease ? 'Increase' : 'Decrease')}
              </span>
...
export default function TradingContractAmountHistory({
  contractId,
  fallbackAdjustments = [],
  fallbackCount = 0,
}) {
  ...
  const detailQuery = useGetTradingContractByIdQuery(numericId, {
    skip: !hasId,
  })
  ...
  const adjustments = useMemo(() => {
    if (filter === 'increase' || filter === 'decrease') {
      return allAdjustments.filter((item) => String(item?.type || '').toLowerCase() === filter)
    }
